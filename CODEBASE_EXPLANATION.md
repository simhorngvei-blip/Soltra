...
│   ├── soltra-camera-node-cv/       ← Experimental CV variant of the camera node
│   ├── soltra-master-hub-cv/        ← Experimental CV variant of the master hub
│   ├── soltra-sensor-node-pcb/      ← PCB design files for the sensor node
│   ├── soltra-sensor-node-test/     ← Test sketches for the sensor node
│   ├── soltra-camera-node-test/     ← Test sketches for the camera node
│   ├── uv-test/                     ← Test sketches for the UV sensor
...
│
├── persona/                         ← Ada's personality/UI component (a React app)
├── docs/                            ← Extra documentation and diagrams
├── designs/                         ← UI design mockups and assets
├── experiments/                     ← Experimental/prototype code (not used in prod)
├── Helios_Final_Report/             ← Academic/project report documents
...
## 👤 `persona/` — Ada's UI Component

**Think of it as:** The visual "face" of Ada, Soltra's AI assistant. It's a standalone React component (built with Vite) that renders Ada's animated avatar. This is embedded inside the SaaS dashboard.

**Key files:**

| File | What it does |
|------|--------------|
| `src/` | The Ada avatar component source code |
| `P3Menu.jsx` | A UI menu component for the Ada persona |
| `package.json` | Dependencies for this micro-app |
...
| `debug.txt` | Developer debug notes. |
