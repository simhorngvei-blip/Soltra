...
<div class="absolute inset-0 w-full h-full flex items-center justify-center bg-black/85 backdrop-blur-md px-4 py-8" ontouchstart={handleTouchStart} ontouchend={handleTouchEnd} transition:fade={{ duration: 200 }}>
  ...
  <div class="relative bg-[#001428]/80 border border-primary/30 border-t-4 border-t-[#ff2a2a] p-8 w-[80%] h-[80%] overflow-y-auto z-10" transition:scale={{ start: 0.9, duration: 250 }}>
    ...
    <div class="flex justify-between items-center mb-6 border-b-2 border-primary pb-3 shrink-0">
      ...
      <button type="button" onclick={() => goto('/')} class="text-[#ff2a2a] hover:text-white transition-colors cursor-pointer">
    ...
    <div class="mb-5">
      ...
      {:else if profiles.length === 0}
        <div class="text-gray-400 text-sm font-mono py-5 text-center">No voice profiles. Clone your first voice below.</div>
      {:else}
        <div class="flex flex-col gap-2">
          {#each profiles as p (p.id)}
            <div class="flex items-center justify-between bg-[#001428]/60 border border-primary/20 border-l-4 border-l-primary p-3" transition:scale>
              <div>
                <div class="text-white text-base tracking-wide">{p.name}</div>
              ...
              <div class="flex gap-2">
                <button
                  ...
                  class="bg-transparent border border-primary/40 text-primary px-3 py-1.5 hover:bg-primary/20 transition-colors disabled:opacity-50 flex items-center justify-center min-w-8 cursor-pointer"
                >
                  {#if testing === p.id}
                ...
                <button
                  ...
                  class="bg-transparent border border-[#ff2a2a]/40 text-[#ff2a2a] px-3 py-1.5 hover:bg-[#ff2a2a]/20 transition-colors cursor-pointer"
                >
    ...
    <!-- Clone Form -->
    {#if !showForm}
      <button type="button" onclick={() => showForm = true} class="bg-[#ff2a2a] text-white border-none py-2.5 px-6 font-anton text-sm tracking-widest flex items-center gap-2 hover:bg-white hover:text-[#ff2a2a] transition-colors w-full justify-center cursor-pointer">
    ...
    {:else}
      <div class="border-t border-primary/20 pt-5" transition:fade>
        ...
        <div class="mb-3">
          <label class="text-sm font-bold text-primary block mb-1">VOICE NAME</label>
          <input
            type="text"
        ...
        <div class="mb-3">
          <label class="text-sm font-bold text-primary block mb-1">REFERENCE TRANSCRIPT (OPTIONAL)</label>
          <textarea
            bind:value={refText}
            placeholder="Transcript of what is said in the audio..."
            rows="3"
            class="w-full bg-[#001428]/80 border border-primary/30 text-white p-3 font-mono text-sm outline-none focus:border-primary transition-colors resize-y"
          ></textarea>
        </div>

        <div class="mb-5">
          <label class="text-sm font-bold text-primary block mb-1">REFERENCE AUDIO (.wav)</label>
          <input
            bind:this={fileInput}
            type="file"
            accept="audio/*"
          ...
          <button
            ...
            class="w-full bg-transparent border border-primary/40 text-primary py-3 font-anton text-sm tracking-widest flex items-center justify-center gap-2 hover:bg-primary/20 transition-colors cursor-pointer"
          >
        ...
        <div class="flex gap-3">
          <button
            ...
            onclick={handleClone}
            disabled={cloning}
            class="flex-1 bg-[#ff2a2a] text-white border-none py-3 font-anton text-base tracking-widest flex items-center justify-center gap-2 hover:bg-white hover:text-[#ff2a2a] transition-colors disabled:opacity-60 cursor-pointer"
          >
            {#if cloning}
          ...
          <button
            ...
            class="bg-transparent border border-primary/40 text-primary py-3 px-6 font-anton text-sm tracking-widest hover:bg-primary/20 transition-colors cursor-pointer"
          >
            CANCEL
          </button>
        </div>
