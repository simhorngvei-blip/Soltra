...
import { Bot, User, X, MessageSquare, Mic, Volume2 } from 'lucide-react';
import { useSoltraTelemetry } from '../hooks/useSoltraTelemetry';
import { generateVoiceboxTTS } from '../utils/ttsService';

const OVERSEER_URL = import.meta.env.VITE_MINI_OVERSEER_URL || 'http://127.0.0.1:8100';

export default function MiniOverseerWidget() {
  ...
  const { telemetry } = useSoltraTelemetry();
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      // 2. Trigger TTS
      try {
        // use 'en' to align with chatterbox expectations, or default kokoro 'en-us' based on your implementation
        const audioBuf = await generateVoiceboxTTS(aiResponse, 'en');
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="px-4 py-1.5 bg-cyan-900/20 border-b border-cyan-900/30 flex justify-between items-center text-[10px] text-cyan-400/70 uppercase tracking-wider">
              <span>WND: {telemetry.wind?.toFixed(1)}m/s</span>
              <span>BAT: {telemetry.battery_pct?.toFixed(0)}%</span>
              <span>SYS: LTM ON</span>
            </div>
