...
//
#define NODE_ID 3   // ← change to 1, 2, 3, or 4 for each board
//
// STEP 2 — TEST MODE
...
read_sensors_label:
  {
    ...
    // BUG 3 FIX: Calibration procedure:
    //
    // LDR (ldr_mult / ldr_off):
    //   1. Place all 4 nodes flat under identical, stable light (e.g. indoor lamp).
    //   2. Open Serial Monitor on each node and record the printed ldr_raw values.
    //   3. Node 1 is the BASELINE. For each other node:
    //        ldr_mult = (node1_reading) / (nodeX_reading)  [corrects gain diff]
    //        ldr_off  = 0  (use offset only if mult alone is insufficient)
    //   4. Example: Node1=1800, Node3=1600  →  ldr_mult = 1800/1600 = 1.125
    //
    // UV (uv_mult / uv_off):
    //   Formula: uv_voltage / 0.1 assumes GUVA-S12SD sensor (0.1V per UV index).
    //   If using a generic UV photodiode, this is likely WRONG — skip UV for now.
    //   Cross-node: repeat same process as LDR under same UV source.
    //
    // Battery:
    //   XIAO ESP32C3 ADC ref ≈ 3.3V, 12-bit (0–4095). Built-in 1:2 divider on A0.
    //   Formula: (raw / 4095.0) * 3.3 * 2.0  is CORRECT for the onboard divider.
    //   Verify with a multimeter: measure actual battery voltage, compare to serial.
    //
    // ─── Measured offsets (update after running calibration procedure) ────────
    ...
    else if (NODE_ID == 2) { ldr_off = +165; uv_off = +0.6f; }
    else if (NODE_ID == 3) { ldr_off = -117; uv_off = +1.9f; }
    else if (NODE_ID == 4) { ldr_mult = 1.0; ldr_off = 0; /* measure and fill in */ }
    ...
    Serial.printf("Node %d | CH:%d | Bat:%.2fV | LDR:%d | Lux:%u | UV:%.2f | IR:%.2f\n",
      NODE_ID, wifi_channel, battery_v, ldr_raw, lux, uv_index, ir_ratio);
