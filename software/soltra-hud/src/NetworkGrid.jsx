...
const ROLES = [
  { text: "PRIMARY", color: "#00d9ff", bg: "rgba(0,217,255,0.12)", border: "rgba(0,217,255,0.5)" },
  { text: "SECONDARY",  color: "#ff2a2a", bg: "rgba(255,42,42,0.12)", border: "rgba(255,42,42,0.5)" },
  { text: "SECONDARY",  color: "#e8c100", bg: "rgba(232,193,0,0.12)", border: "rgba(232,193,0,0.5)" },
];

const INITIAL_ITEMS = [
  {
    id: "alpha", label: "NODE ALPHA", handle: "@192.168.1.101", href: "#", icon: "🌐", barIcon: icon1, bars: 1, newBars: [0], counts: ["ONLINE"],
    links: ["system.soltra/ping/alpha"],
    stats: [
      { tag: "LAT", value: "14ms", color: "#00d9ff" },
      { tag: "PKT", value: "99%",  color: "#00d9ff" },
    ],
  },
  {
    id: "beta", label: "NODE BETA", handle: "@192.168.1.102", href: "#", icon: "🌐", barIcon: icon2, bars: 5, newBars: [1, 2], counts: ["SYNC", "SYNC", "ERR", "SYNC", "SYNC"],
    links: ["system.soltra/log/b1", "system.soltra/log/b2", "system.soltra/log/b3", "system.soltra/log/b4", "system.soltra/log/b5"],
    stats: [
      { tag: "LAT", value: "22ms", color: "#ff2a2a" },
      { tag: "PKT", value: "92%",  color: "#ff2a2a" },
    ],
  },
  {
    id: "gamma", label: "NODE GAMMA", handle: "@192.168.1.103", href: "#", icon: "🌐", barIcon: icon3, bars: 7, newBars: [0, 3, 5, 6], counts: ["IDLE", "IDLE", "IDLE", "IDLE", "IDLE", "IDLE", "IDLE"],
    links: ["system.soltra/log/g1", "system.soltra/log/g2", "system.soltra/log/g3", "system.soltra/log/g4", "system.soltra/log/g5", "system.soltra/log/g6", "system.soltra/log/g7"],
    stats: [
      { tag: "LAT", value: "18ms", color: "#e8c100" },
      { tag: "PKT", value: "98%",  color: "#e8c100" },
    ],
  },
];
  ...
  useEffect(() => {
    const int = setInterval(() => {
      setItems(prev => prev.map(item => ({
        ...item,
        stats: [
          { ...item.stats[0], value: Math.max(10, parseInt(item.stats[0].value) + (Math.random() - 0.5) * 6).toFixed(0) + "ms" },
          { ...item.stats[1], value: Math.max(80, Math.min(100, parseInt(item.stats[1].value) + (Math.random() - 0.3) * 2)).toFixed(0) + "%" }
        ]
      })));
    }, 2000);
...
export default function Socials() {
  const ITEMS = useNetworkTelemetry();
  ...
  return (
    <div id="menu-screen">
      ...
      <style>{`
        ...
        .sc-bar-outer.active .sc-bar-cyan { height: 90px; }
        .sc-bar-outer.mounted { transform: translateX(0); }
        .sc-bar-outer:nth-child(1) { transition-delay: 0ms; }
        .sc-bar-outer:nth-child(2) { transition-delay: 80ms; }
        .sc-bar-outer:nth-child(3) { transition-delay: 160ms; }

        /* cyan underlay — peeks out below the bar when active */
        .sc-bar-cyan {
          position: absolute;
          top: 0; left: 0;
          width: 45vw;
        ...
        .sc-bar-outer.active .sc-bar-cyan { opacity: 1; }

        /* dark fill — skewed parallelogram on the right 25% */
        .sc-bar-fill {
          position: absolute;
          inset: 0;
          width: 100%;
        ...
        .sc-bar-outer.active .sc-label { color: #00d9ff; }
        ...
        .sc-bar-outer.active .sc-stat-num { color: #00d9ff; }
