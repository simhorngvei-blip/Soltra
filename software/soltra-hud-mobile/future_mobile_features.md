...
*   **Self-Hosting (Current Method):** We currently use the `soltra-tts` server running the open-source **Kokoro ONNX** model on local hardware. This means voice generation is permanently free and entirely private.
*   **API Free Tiers:** If you prefer to offload generation to commercial APIs (like Cartesia or ElevenLabs), they typically offer generous free tiers (~10,000 characters/month) which is more than enough for a daily report prototype.
*   **Implementation Strategy:** Our Kokoro TTS server already supports multiple voices. During the Daily Report routine, the mobile app sends text to the server tagged with a specific voice ID (e.g., `af_bella`) to generate the audio dynamically.
...
**How to do it for FREE:**
*   **HiveMQ Serverless Cloud (Current Method):** We currently use HiveMQ which offers a permanently free tier for up to 100 connected devices and 10GB of data per month. Ideal for our deployment.
*   **Oracle Cloud "Always Free" VPS (Alternative):** Spin up an always-free Oracle cloud server and install an open-source broker like **Eclipse Mosquitto**. This gives you an enterprise-grade, private cloud MQTT broker for $0/month if you outgrow HiveMQ.
