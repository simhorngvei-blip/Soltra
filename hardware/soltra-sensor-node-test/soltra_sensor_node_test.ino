// ═════════════════════════════════════════════
// SOLTRA HARDWARE CONFIG — SENSOR NODE (NO SLEEP TEST)
...
/*
 * PROJECT SOLTRA — Sensor Node (XIAO ESP32C3) - CONTINUOUS TEST (NO SLEEP)
 *
...
const int UV_PIN  = A2;
const int LED_PIN = 5; // GPIO 5 is D3 on XIAO ESP32C3
...
int active_wifi_channel = 1; // Used in loop to print
...
void setup() {
  ...
  Serial.printf("\n==========================================\n");
  Serial.printf("   SOLTRA SENSOR NODE %d (NO SLEEP TEST)\n", NODE_ID);
  Serial.printf("==========================================\n");
...
void loop() {
  ...
  Serial.printf("Node %d | CH:%d | Bat:%.2fV | LDR:%d | UV:%.2f | IR:%.2f\n",
    NODE_ID, active_wifi_channel, battery_v, ldr_raw, uv_index, ir_ratio);

  esp_now_send(HUB_MAC, (uint8_t*)&txData, sizeof(txData));

  digitalWrite(LED_PIN, LOW); delay(50); digitalWrite(LED_PIN, HIGH);

  delay(2000); // 2 second delay instead of deep sleep
}
